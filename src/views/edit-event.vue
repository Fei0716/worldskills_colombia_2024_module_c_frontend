<template>
  ...
  <div class=" bg-slate-300 w-[800px] flex flex-col p-4 rounded-ee rounded-bl">
    ...
    <form action=""  @submit.prevent="editEvent">
      <div class=" bg-slate-300 w-[380px] flex flex-col p-4 rounded-ee rounded-bl">
        ...
        <input type="text" placeholder="name" :value="selectedEvent.name" class="bg-slate-400 placeholder-slate-600 text-slate-600 input input-bordered w-full mb-2" readonly/>
        <label class="text-slate-600 font-semibold">Date:</label>
        <input type="date" placeholder="date"  class="bg-slate-400 placeholder-slate-600 text-slate-600 input input-bordered w-full mb-2" v-model="formattedDate"/>
        <label class="text-slate-600 font-semibold">Venue:</label>
        <select class="select bg-slate-400 placeholder-slate-600 text-slate-600 input input-bordered w-full mb-2" v-model="date">
          <template v-for="v of venues" >
            <option :value="v.id" :key="v.id" v-if="v.id === selectedEvent.venue.id" selected>{{v.name}}</option>
            <option :value="v.id" :key="v.id" v-else disabled>{{v.name}}</option>
          </template>
        </select>
...
async function getEvents(){
  try{
    ...
    formattedDate  = computed({
      get(){
        //format it
        return selectedEvent.value.date.toString().split('').slice(0 , 10).join('');
      },
      set(value){
        //convert the object back to date object
        selectedEvent.value.date = value;
      },
    });
